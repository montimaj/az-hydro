...
def create_all_raster_maps(skip_maps: set[str] | None = None) -> None:
    ...
    # Reads the CAP_Scenario CSVs produced upstream by Step 3b
    # (run_cap_scenario_analysis) and renders all five spatial-map
    # outputs: basin/pixel cumulative ΔGW central, basin/pixel CV
    # signal-to-noise per scenario, and a single 2-panel σ_cum
    # context figure.  Outputs land under
    # ``Raster_Maps/CAP_Scenario/`` so the entire CAP-scenario
    # spatial suite sits next to the other era / σ map figures.
    if 'cap-scenario-maps' not in skip_maps:
        cap_scenario_csv_dir = os.path.join(
            prediction_dir, 'Uncertainty', 'CAP_Scenario',
        )
        if os.path.isdir(cap_scenario_csv_dir):
            cap_maps_out = os.path.join(maps_dir, 'CAP_Scenario')
            cap_geojson = os.path.join(
                VECTOR_DIR, 'CAP', 'CAP_Service_Area.geojson',
            )
            create_cap_scenario_spatial_maps(
                cap_scenario_csv_dir=cap_scenario_csv_dir,
                output_dir=cap_maps_out,
                basin_shp=AZ_GW_BASIN,
                total_gw_dir=os.path.join(
...
def create_cap_scenario_spatial_maps(
    cap_scenario_csv_dir: str,
    output_dir: str,
    basin_shp: str,
    total_gw_dir: str,
    ...
    *,
    ...
    year_windows: list[tuple[int, int]] | None = None,
    cap_service_area_geojson: str | None = None,
) -> None:
    """Render the full CAP scenario spatial-map suite to *output_dir*.

    Five outputs (all under ``output_dir``):
      1. ``CAP_Scenario_Basin_Drawdown.png`` — 2×4 grid of per-basin
         cumulative ΔGW per scenario (10⁶ m³ + AF secondary axis).
      2. ``CAP_Scenario_Pixel_Drawdown.png`` — pixel-level analogue
         (basin Δ × per-pixel ML Total_GW share — pro-rata, NOT a
    ...
    Reads CSVs / rasters generated by Step 3b
    (``uncertaintyops.run_cap_scenario_analysis``) — must run after
    Step 3b in any pipeline invocation.

    Args:
        cap_scenario_csv_dir: Directory containing
            ``CAP_Scenario_Delta.csv`` (typically
            ``Uncertainty/CAP_Scenario/``).
        output_dir: Destination directory for all rendered figures
            and the per-scenario per-pixel rasters.
        basin_shp: AZ groundwater basin shapefile path.
        total_gw_dir: Directory with ``Total_GW_{year}_mm.tif`` rasters.
        sigma_raster_dir: ``Uncertainty/Sigma_Total/Rasters/`` containing
            ``Sigma_Total_Total_GW_mm_{year}.tif``.
        unc_dir: ``Uncertainty/`` root (used to locate per-component
            ``Sigma_<comp>/Basin_Sigma_<comp>_Total_GW.csv`` files).
        year_window: Inclusive cumulative window (default 2027–2060
            to match the WestWater 2026 comparison).
    """
    from hydrolibs import uncertaintyops as uncops
    delta_path = os.path.join(
        cap_scenario_csv_dir, 'CAP_Scenario_Delta.csv',
    )
    if not os.path.isfile(delta_path):
        logger.warning(
            'CAP scenario maps: %s not found — skipping', delta_path,
        )
        return
    makedirs(output_dir)
    delta_df = pd.read_csv(delta_path)

    # Render every figure / raster / CSV per window.  Default keeps
    # the WestWater 2027–2060 anchor window AND adds the full
    # projection 2027–2099 so reviewers can see both perspectives.
    # Filenames are auto-suffixed with the window
    # (e.g. ``CAP_Scenario_Basin_Drawdown_2027_2060.png`` vs
    # ``..._2027_2099.png``).
    if year_windows is None:
        year_windows = [year_window, (2027, 2099)]
    # De-duplicate while preserving order
    seen_windows: set[tuple[int, int]] = set()
    deduped_windows: list[tuple[int, int]] = []
    for w in year_windows:
        if w not in seen_windows:
            seen_windows.add(w)
            deduped_windows.append(w)

    for yw in deduped_windows:
        logger.info(
            '  CAP scenario spatial maps for window %d–%d',
            yw[0], yw[1],
        )

        # 1. Basin drawdown (central) — clipped to CAP service area
        try:
            uncops._plot_cap_scenario_basin_drawdown(
                delta_df, basin_shp, output_dir, year_window=yw,
                cap_service_area_geojson=cap_service_area_geojson,
            )
        except Exception as exc:  # noqa: BLE001
            logger.warning(
                '  CAP scenario basin drawdown failed: %s', exc,
            )

        # 2. Pixel drawdown (central)
        pixel_paths: dict[str, str] = {}
        if os.path.isdir(total_gw_dir):
            try:
                pixel_paths = uncops._generate_cap_scenario_pixel_rasters(
                    delta_df, basin_shp, total_gw_dir, output_dir,
                    year_window=yw,
                    cap_service_area_geojson=cap_service_area_geojson,
                )
                if pixel_paths:
                    uncops._plot_cap_scenario_pixel_drawdown(
                        pixel_paths, basin_shp, output_dir,
                        year_window=yw,
                        cap_service_area_geojson=cap_service_area_geojson,
                    )
            except Exception as exc:  # noqa: BLE001
                logger.warning(
                    '  CAP scenario pixel drawdown failed: %s', exc,
                )
        else:
            logger.info(
                '  CAP scenario pixel drawdown skipped — Total_GW '
                'raster directory not found at %s', total_gw_dir,
            )

        # 3. σ_cum context (basin + pixel) — basin σ ALSO computed
        # restricted to the basin × CAP-pixel intersection (used by
        # the CV map below); the σ_cum context figure shows the
        # full-basin σ for comparison.
        basin_sigma_yearly = uncops._load_basin_sigma_total_gw_yearly(
            unc_dir, yw,
        )
        basin_sigma_cum_full = uncops._compute_basin_sigma_cum(
            basin_sigma_yearly, yw,
        )
        if cap_service_area_geojson and os.path.isfile(
            cap_service_area_geojson,
        ):
            # Per-year per-basin CAP-restricted σ CSV — window-suffixed
            cap_sigma_csv = os.path.join(
                cap_scenario_csv_dir,
                f'Basin_Sigma_CAP_Restricted_Total_GW_'
                f'{yw[0]}_{yw[1]}.csv',
            )
            try:
                uncops._save_cap_restricted_basin_sigma_csv(
                    sigma_raster_dir, basin_shp,
                    cap_service_area_geojson, yw,
                    cap_sigma_csv,
                )
            except Exception as exc:  # noqa: BLE001
                logger.warning(
                    '  CAP-restricted basin σ CSV write failed: %s',
                    exc,
                )
            basin_sigma_cum_capclip = (
                uncops._compute_basin_sigma_cum_cap_restricted(
                    sigma_raster_dir, basin_shp,
                    cap_service_area_geojson, yw,
                )
            )
            ...
            basin_sigma_cum_capclip = {}
        pix_sigma_result = uncops._compute_pixel_sigma_cum(
            sigma_raster_dir, yw,
            cap_service_area_geojson=cap_service_area_geojson,
        )
        if pix_sigma_result is not None:
    ...
    logger.info(
        'CAP scenario spatial maps saved to %s', output_dir,
    )
