...
The 2019 ADWR statewide GW share comparison is the loosest of the agency cross-checks at 3 percentage points (model 44 % vs ADWR 41 %, [MAP Arizona Dashboard](https://mapazdashboard.arizona.edu/article/arizonas-water-use-sector)) and is included for completeness. The training labels themselves come *only* from metered ADWR records in the ten AMA/INA management areas — the model has never seen a single training point from the ~25 unmetered basins that contribute roughly 30 % of the predicted statewide volume.

**Partition-time calibration to historical anchor points.** While the ML model is trained without agency labels, the deterministic partitioning that splits the ML-predicted total into Irrigation/Non-Irrigation × GW/SW categories was iteratively calibrated against published statewide aggregates spanning 1915–2017 (USGS Open-File Report 94-476 pre-1938 GW estimates, USGS Circulars 115/398/456/556/676/765/1001/1004/1081/1200/1268/1344/1405/1441 for 1950–2015, USBR 1950/1970 narratives, and ADWR Annual Report 2016 statewide totals). The calibration tunes era-dependent partition parameters — well-density blend ramps for the unregistered pre-GMA well era, era-specific `gw_weight` in the density-ratio split (1.0 pre-GMA → 0.1 post-CAP), the ag-era `irr_cap = 1 − urban_frac` override for 1970–1980, and Gaussian SW-smoothing radius (σ=4 pre-GMA, σ=16 post-GMA) — to reproduce the documented historical GW/SW and Irrigation/Non-irrigation shares. **The XGBoost predictions of total annual pumping themselves remain uncalibrated to any agency aggregate**; only the post-hoc partitioning of those totals into the eight reporting categories incorporates the historical anchors. Full calibration tables and design principles are documented in [`azhydro/README.md` § Calibration](azhydro/README.md#calibration).
