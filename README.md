...
A full setup with input data and a complete pipeline run requires approximately **210 GB**:

| Component | Size | Notes |
|---|---|---|
| **Inputs total (Zenodo)** | ~9 GB | Downloaded separately from the Git repository — see [`Data/README.md`](Data/README.md) for the per-directory inventory and the external-source citations for HarDWR / GRAIN / NHM / Reitz / PS / Crop Surveys (must be obtained from USGS ScienceBase / Zenodo) |
...
| **Code & figures** | ~3 MB | Python source, GEE scripts, README figures (`docs/images/`) |

Most of the ~200 GB of outputs is generated by the pipeline from the
~9 GB Zenodo inputs.  Step 2 (full model evaluation across 5
