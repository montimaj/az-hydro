...
**What's new.** **To our knowledge, this is the first study globally** that combines all four of the following in one self-consistent framework: (1) a 204-year continuous statewide water-use dataset spanning hindcast (1896–1983), historical (1984–2025), and projection (2026–2099) eras at 2 km annual resolution; (2) the first publicly available statewide irrigation consumptive use dataset for Arizona at this combination of resolution and temporal coverage; (3) a per-pixel, per-year surface-water capture index that apportions GW pumping into stream-depletion vs. storage-mining shares at 2 km annual resolution — work normally done one basin at a time with calibrated transient MODFLOW–SFR models; and (4) a density-ratio GW/SW partitioning driven by HarDWR water-rights records and canal-weighted streamflow that uses locally observable infrastructure rather than global statistical proxies. No prior study we are aware of provides this combination of spatial coverage, temporal span, and per-pixel/per-well disaggregation for any U.S. state, and the integrated framework is portable to any state with comparable agency metering and water-rights records.
